...
local function configurator()
	...
	local data = ecs.universalWindow("auto", "auto", 40, 0xeeeeee, true, {"EmptyLine"}, {"CenterText", 0x880000, "Wow, ebana!"}, {"EmptyLine"}, {"WrappedText", 0x262626, "Welcome to the multi-monitor configuration. You need to specify the number of monitors on the width and height that you want to merge, and select the desired scale."}, {"EmptyLine"}, {"Input", 0x262626, 0x880000, "Width"}, {"Input", 0x262626, 0x880000, "Высота"},  {"Slider", 0x262626, 0x880000, 1, 100, 100, "Scale: ", "%"}, {"EmptyLine"}, {"Button", {ecs.colors.orange, 0xffffff, "confirm"}, {0x777777, 0xffffff, "Cancel"}})
	local width, height, scale = tonumber(data[1]), tonumber(data[2]), tonumber(data[3]) / 100
	if data[4] == "Cancel" then
		ecs.prepareToExit()
...
local function drawBigImageFromOCIFRawFile(x, y, path)
	...
	print("Read the file signature: " .. signature)
	local encodingMethod = string.byte(file:read(1))
	print("We read the encoding method: " .. encodingMethod)

	if encodingMethod ~= 5 then error("Unsupported encoding method: " .. encodingMethod) end
