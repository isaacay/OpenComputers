	...
	if obj["Elements"] then
		for key, val in pairs(obj["Elements"]) do
			if ecs.clickedAtArea(e[3], e[4], obj["Elements"][key][1], obj["Elements"][key][2], obj["Elements"][key][3], obj["Elements"][key][4]) then
				...
				--ecs.error("Number of objects: "..#obj["Elements"]..", clicked on an object number = "..tostring(key)..", #Data = "..#data..", sData ="..sData)
...
-- 	local e = {event.pull("touch")}
-- 	ecs.prepareToExit()
-- 	local action = context.menu(e[3], e[4], {"Show content"}, "-", {"Cut", false, "^X"}, {"Copy", false, "^C"}, {"insert", true, "^V"}, "-", {"Rename"}, {"Create Shortcut"}, {"Add to archive"}, "-", {"Delete", false, "⌫"})
-- 	ecs.prepareToExit()
-- 	print("You chose = "..tostring(action))
-- end

--local action = context.menu(6, 2, {(function() if showHiddenFiles then return "Hide Hidden Files" else return "Show hidden files" end end)()}, {(function() if showSystemFiles then return "Hide system files" else return "Show system files" end end)()}, "-", {(function() if showFileFormat then return "Hide file format" else return "Show file format" end end)()})
